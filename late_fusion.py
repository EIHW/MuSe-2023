...
def parse_args():
    ...
    parser.add_argument('--personalised', nargs='+', required=False, help=f'Personalised model IDs for {PERSONALISATION}, '
    ...
    if args.task==PERSONALISATION:
        args.prediction_dirs = [os.path.join(PREDICTION_FOLDER, PERSONALISATION, args.emo_dim, args.model_ids[i], args.personalised[i]) for i in range(len(args.model_ids))]
    else:
