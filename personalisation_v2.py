...
from train import train_personalised_models_v2
...
def personalise(initial_model, feature, emo_dim, temp_dir, paths, normalize, win_len, hop_len, epochs, lr, use_gpu, loss_fn,
    ...
    personalised_cps = train_personalised_models_v2(initial_model=initial_model, temp_dir=temp_dir, data_loaders=data_loaders, subject_ids=test_ids, epochs=epochs,
...
if __name__ == '__main__':
    args = parse_args()
    initial_model = torch.load(args.model_file, map_location=config.device)
    ...
    if not args.eval_personalised:
        ...
        val_score, test_score = personalise(initial_model=initial_model, feature=args.feature, emo_dim=args.emo_dim, temp_dir=pers_dir, paths=args.paths,
